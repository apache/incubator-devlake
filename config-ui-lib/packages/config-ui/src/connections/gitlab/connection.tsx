...
import { GitLabConnectionList } from './list';
import { GitLabConnectionForm } from './form';
import type { GitLabItemType, GitLabPayloadType } from './typed';
import * as API from './api';
import * as S from './styled';

export const GitLabConnection = () => {
  ...
  const handleDelete = async (row: GitLabItemType) => {
  ...
  const handleUpdate = (row: GitLabItemType) => {
    setUpdateObj(row);
    setModalType('edit');
    handleShowModal();
  };

  const handleSubmit = async (values: GitLabPayloadType) => {
    const request =
      modalType === 'add'
        ? () => API.create(values)
...
GitLabConnection.Form = GitLabConnectionForm;
GitLabConnection.List = GitLabConnectionList;
