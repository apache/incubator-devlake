...
SHA := $(shell if [ -d .git ] || [ -d ../.git ]; then git show -s --format=%h; else echo "default_SHA"; fi)
TAG := $(shell if [ -d .git ] || [ -d ../.git ]; then git tag --points-at HEAD; else echo "default_TAG"; fi)
IMAGE_REPO ?= "apache"
VERSION = $(TAG)@$(SHA)
