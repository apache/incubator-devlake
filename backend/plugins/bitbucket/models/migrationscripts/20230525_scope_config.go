...
type scopeConfig20230525 struct {
	Entities []string `gorm:"type:json" json:"entities"`
}

func (scopeConfig20230525) TableName() string {
	return "_tool_bitbucket_scope_configs"
}

func (u *renameTr2ScopeConfig) Up(baseRes context.BasicRes) errors.Error {
	db := baseRes.GetDal()
	err := db.RenameColumn("_tool_bitbucket_repos", "transformation_rule_id", "scope_config_id")
	if err != nil {
		return err
	}
	err = db.RenameTable("_tool_bitbucket_transformation_rules", "_tool_bitbucket_scope_configs")
	if err != nil {
		return err
	}
	return migrationhelper.AutoMigrateTables(baseRes, &scopeConfig20230525{})
}

func (*renameTr2ScopeConfig) Version() uint64 {
	return 20230525161352
}
