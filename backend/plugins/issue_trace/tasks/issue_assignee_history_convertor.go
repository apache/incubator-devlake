...
func ConvertIssueAssigneeHistory(taskCtx plugin.SubTaskContext) errors.Error {
	...
	insertor := helper.NewBatchSaveDivider(taskCtx, utils.BATCH_SIZE, rawTableIssueChangelogs, scopeId)
	...
	clauses := []dal.Clause{
		...
		dal.Where("field_name = 'assignee' AND board_issues.board_id = ?", scopeId),
		...
		Convert: func(inputRow interface{}) ([]interface{}, errors.Error) {
			...
			if row.IssueId != currentIssue {
				if len(currentLogs) > 0 {
					historyRows := buildActiveAssigneeHistory(currentLogs, scopeId)
...
func buildActiveAssigneeHistory(logs []AssigneeChangelog, scopeId string) []*models.IssueAssigneeHistory {
