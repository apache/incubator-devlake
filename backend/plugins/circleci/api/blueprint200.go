...
func makePipelinePlanV200(
	subtaskMetas []plugin.SubTaskMeta,
	scopeDetails []*srvhelper.ScopeDetail[models.CircleciProject, models.CircleciScopeConfig],
...
func makeScopesV200(
	...
	idgen := didgen.NewDomainIdGenerator(&models.CircleciProject{})
	for _, scopeDetail := range scopeDetails {
		scope, scopeConfig := scopeDetail.Scope, scopeDetail.ScopeConfig
		id := idgen.Generate(connection.ID, scope.Slug)

		// add cicd_scope to scopes
		if utils.StringsContains(scopeConfig.Entities, plugin.DOMAIN_TYPE_CICD) {
			scopes = append(scopes, devops.NewCicdScope(id, scope.Slug))
		}
	}
