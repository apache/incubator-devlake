...
func ConvertAccounts(taskCtx plugin.SubTaskContext) errors.Error {
	...
	db := taskCtx.GetDal()
	clauses := []dal.Clause{
		dal.From(&models.CircleciAccount{}),
	...
	converter, err := helper.NewDataConverter(helper.DataConverterArgs{
		...
		InputRowType:       reflect.TypeOf(models.CircleciAccount{}),
		Input:              cursor,
		Convert: func(inputRow interface{}) ([]interface{}, errors.Error) {
			userTool := inputRow.(*models.CircleciAccount)
			account := &crossdomain.Account{
				DomainEntity: domainlayer.DomainEntity{
					Id: getAccountIdGen().Generate(data.Options.ConnectionId, userTool.Id),
				},
				UserName:     userTool.Name,
				FullName:     userTool.Name,
				AvatarUrl:    userTool.AvatarUrl,
				Organization: userTool.Slug,
			}
