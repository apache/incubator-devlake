...
// scope config whose IssueTypeIncident pattern matches the "Bug" label, ENG-1
// (labeled "Bug") is converted to an INCIDENT, while issues without a matching
// label stay REQUIREMENT. This is what feeds DORA's change-failure-rate metric.
func TestLinearIssueIncidentMapping(t *testing.T) {
	...
	taskData := &tasks.LinearTaskData{
		...
		ScopeConfig: &models.LinearScopeConfig{
			IssueTypeIncident: "^Bug$",
		},
	}

	// extract issues + inline labels from raw
	...
	// accounts needed so the convertor can resolve assignee/creator names
	dataflowTester.ImportCsvIntoTabler("./snapshot_tables/_tool_linear_accounts.csv", &models.LinearAccount{})

	// convert -> domain issues; ENG-1 (Bug label) must be INCIDENT, others REQUIREMENT
	...
	dataflowTester.VerifyTableWithOptions(ticket.Issue{}, e2ehelper.TableOptions{
		CSVRelPath:  "./snapshot_tables/issues_incident.csv",
