...
func CollectEpics(taskCtx plugin.SubTaskContext) errors.Error {
	...
	batchSize := 100
	if strings.ToLower(string(data.JiraServerInfo.DeploymentType)) == string(models.DeploymentServer) && len(data.JiraServerInfo.VersionNumbers) == 3 && data.JiraServerInfo.VersionNumbers[0] <= 8 {
		batchSize = 1
	}
	...
	if strings.ToLower(string(data.JiraServerInfo.DeploymentType)) == string(models.DeploymentServer) {
