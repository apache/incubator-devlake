...
var ConvertPrLabelsMeta = plugin.SubTaskMeta{
	...
	Description:      "Convert tool layer table azuredevops_pull_request_labels into domain layer table pull_request_labels",
...
func ConvertPrLabels(taskCtx plugin.SubTaskContext) errors.Error {
	...
	clauses := []dal.Clause{
		...
		dal.Join(`left join _tool_azuredevops_pull_requests on
			_tool_azuredevops_pull_requests.azuredevops_id = _tool_azuredevops_pull_request_labels.pull_request_id`),
		dal.Where(`_tool_azuredevops_pull_requests.repository_id = ?
			and _tool_azuredevops_pull_requests.connection_id = ?`,
