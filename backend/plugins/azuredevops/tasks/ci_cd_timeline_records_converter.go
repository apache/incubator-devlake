...
func ConvertApiTimelineRecords(taskCtx plugin.SubTaskContext) errors.Error {
	...
	clauses := []dal.Clause{
		...
		dal.Join(`left join _tool_azuredevops_builds
			on _tool_azuredevops_builds.azuredevops_id =
			_tool_azuredevops_timeline_records.build_id`),
		dal.Where("_tool_azuredevops_builds.repository_id = ? and _tool_azuredevops_builds.connection_id = ?", data.Options.RepositoryId,
