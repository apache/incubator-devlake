...
func (u *renameTr2ScopeConfig) Up(baseRes context.BasicRes) errors.Error {
	db := baseRes.GetDal()
	err := db.RenameColumn("_tool_opsgenie_services", "transformation_rule_id", "scope_config_id")
	if err != nil {
		return err
	}
	err = db.RenameTable("_tool_opsgenie_transformation_rules", "_tool_opsgenie_scope_configs")
	if err != nil {
		return err
	}
...
func (*renameTr2ScopeConfig) Name() string {
	return "rename transformation rule to scope config for opsgenie"
}
