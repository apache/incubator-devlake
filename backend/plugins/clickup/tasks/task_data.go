...
// ClickUpTaskData is the shared context handed to every ClickUp subtask.
type ClickUpTaskData struct {
	Options   *ClickUpOptions
	ApiClient *api.ApiAsyncClient
	TimeAfter *time.Time
	// ScopeConfig carries the resolved scope config (status + type mapping).
	// Never nil: PrepareTaskData defaults it to an empty config.
	ScopeConfig *models.ClickUpScopeConfig
}

type ClickUpApiParams models.ClickUpApiParams
