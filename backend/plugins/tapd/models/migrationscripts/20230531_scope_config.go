...
func (u *renameTr2ScopeConfig) Up(baseRes context.BasicRes) errors.Error {
	db := baseRes.GetDal()
	err := db.RenameColumn("_tool_tapd_workspaces", "transformation_rule_id", "scope_config_id")
	if err != nil {
		return err
	}
	err = db.RenameTable("_tool_tapd_transformation_rules", "_tool_tapd_scope_configs")
	if err != nil {
		return err
	}
