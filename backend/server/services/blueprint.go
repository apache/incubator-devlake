...
func CreateBlueprint(blueprint *models.Blueprint) errors.Error {
	...
	err = services.NewBlueprintManager(db).SaveDbBlueprint(blueprint)
	if err != nil {
		return err
	}
...
func GetBlueprints(query *BlueprintQuery) ([]*models.Blueprint, int64, errors.Error) {
	blueprints, count, err := services.NewBlueprintManager(db).GetDbBlueprints(&services.GetBlueprintQuery{
		Enable:      query.Enable,
...
func GetBlueprint(blueprintId uint64) (*models.Blueprint, errors.Error) {
	blueprint, err := services.NewBlueprintManager(db).GetDbBlueprint(blueprintId)
	if err != nil {
...
func GetBlueprintByProjectName(projectName string) (*models.Blueprint, errors.Error) {
	...
	blueprint, err := services.NewBlueprintManager(db).GetDbBlueprintByProjectName(projectName)
	if err != nil {
...
func ReloadBlueprints(c *cron.Cron) errors.Error {
	enable := true
	isManual := false
	blueprints, _, err := services.NewBlueprintManager(db).GetDbBlueprints(&services.GetBlueprintQuery{
		Enable:   &enable,
		IsManual: &isManual,
	})
	if err != nil {
