...
const Pipelines = (props) => {
  ...
  const [maxPage, setMaxPage] = useState(
    Math.ceil(filteredPipelines.length / perPage)
  )
  ...
  const getPipelineCountByStatus = useCallback(
    (status) => {
      return status === 'all'
        ? pipelines.length
  ...
  const restartPipeline = useCallback(
    (tasks = []) => {
      ...
      console.log(
        '>>> RESTARTING PIPELINE WITH EXISTING CONFIGURATION!!',
  ...
  useEffect(() => {
    ...
    setMaxPage(
      filteredPipelines.length <= perPage
        ? 1
        : Math.ceil(filteredPipelines.length / perPage)
    )
    ...
    <>
              ...
              <div style={{ display: 'flex' }}>
                <div>
                    ...
                    >
                      ...
                      <>
                        ...
                        >
                          ...
                          <p>
                            Need Help? &mdash; Manage, Stop running and Restart
                            failed pipelines. Access{' '}
                            <strong>Task Progress</strong> and Activity for all
                            your pipelines.
                          </p>
                        </div>
                      </>
                  ...
                  <p className='page-description mb-0'>
                    Manage Job Activity and see duration for all your pipeline
                    runs.
                  </p>
                  <p className=''>
                    The most recent runs are shown first, filter by key status
                    types.
                  </p>
                </div>
            ...
            {(isFetchingAll || !isFetchingAll) && (
              <>
                ...
                >
                  ...
                  >
                    <Button
                      ...
                      intent={activeStatus === 'all' ? 'primary' : null}
                    ...
                    >
                      ...
                      <Tag className='tag-data-count'>
                        {getPipelineCountByStatus('all')}
                      </Tag>
                    </Button>
                    <Button
                      className='btn-pipeline-filter'
                      intent={
                        activeStatus === 'TASK_RUNNING' ? 'primary' : null
                      }
                      onClick={() => setActiveStatus('TASK_RUNNING')}
                    >
                      <span
                        style={{
                          marginRight: '10x',
                      ...
                      <Tag className='tag-data-count'>
                        {getPipelineCountByStatus('TASK_RUNNING')}
                      </Tag>
                    </Button>
                    <Button
                      className='btn-pipeline-filter'
                      intent={
                        activeStatus === 'TASK_COMPLETED' ? 'primary' : null
                      }
                      onClick={() => setActiveStatus('TASK_COMPLETED')}
                    >
                      <span
                        style={{
                          marginRight: '10x',
                      ...
                      <Tag className='tag-data-count'>
                        {getPipelineCountByStatus('TASK_COMPLETED')}
                      </Tag>
                    </Button>
                    <Button
                      className='btn-pipeline-filter'
                      intent={activeStatus === 'TASK_FAILED' ? 'primary' : null}
                      onClick={() => setActiveStatus('TASK_FAILED')}
                    >
                      <span
                        style={{
                          display: 'flex',
                      ...
                      >
                        <Icon
                          icon='warning-sign'
                          size={14}
                          style={{ justifySelf: 'center', marginRight: '10px' }}
                        />
                        Failed&nbsp;
                        <Tag className='tag-data-count'>
                          {getPipelineCountByStatus('TASK_FAILED')}
                        </Tag>
                      </span>
                    </Button>
                  </ButtonGroup>
                ...
                >
                  ...
                  >
                    <table
                      className='bp3-html-table bp3-html-table-bordered pipelines-table'
                      style={{ width: '100%' }}
                    >
                      <thead>
                        <tr>
                          ...
                          <th
                            style={{ minWidth: '94px', whiteSpace: 'nowrap' }}
                          >
                            Duration
                          </th>
                          <th
                            style={{ minWidth: '104px', whiteSpace: 'nowrap' }}
                          >
                            Status
                          </th>
                          <th
                            style={{ minWidth: '92px', whiteSpace: 'nowrap' }}
                          />
                        </tr>
                      </thead>
                      <tbody>
                        {!isFetchingAll &&
                          ...
                          pagedPipelines.map((pipeline, pIdx) => (
                            <tr
                              key={`pipeline-row-${pIdx}`}
                              className={
                                pipeline?.status === 'TASK_FAILED'
                                  ? 'pipeline-row pipeline-failed'
                                  : 'pipeline-row'
                              }
                            ...
                            >
                              ...
                              >
                                <Tooltip
                                  content={`Pipeline Run ID #${pipeline.ID}`}
                                ...
                                >
                                  <a
                                    ...
                                    onClick={() =>
                                      history.push(
                                        `/pipelines/activity/${pipeline.ID}`
                                      )
                                    }
                                  >
                                    {pipeline.ID}
                              ...
                              >
                                ...
                                >
                                  ...
                                  {parseInt(pipeline.blueprintId, 10) > 0 && (
                                    <Tooltip
                                      content={`Blueprint ID ${pipeline.blueprintId}`}
                                    >
                                      <Icon
                                        icon='bold'
                                        color={Colors.BLUE4}
                              ...
                              >
                                ...
                                {pipeline.status === 'TASK_RUNNING'
                                  ? dayjs(pipeline.beganAt).toNow(true)
                                  : pipeline.finishedAt == null
                                  ? 'N/A'
                                  : dayjs(pipeline.finishedAt).from(
                                      pipeline.beganAt,
                                      true
                                    )}
                              </td>
                              <td
                                className='cell-status no-user-select'
                                style={{
                              ...
                              >
                                ...
                                {pipeline.status === 'TASK_FAILED' && (
                                  <strong style={{ color: Colors.RED5 }}>
                                    Failed
                                  </strong>
                                )}
                                {pipeline.status === 'TASK_COMPLETED' && (
                                  <strong style={{ color: Colors.GREEN5 }}>
                                    Complete
                                  </strong>
                                )}
                                {pipeline.status === 'TASK_RUNNING' && (
                                  <strong style={{ color: Colors.BLUE5 }}>
                                    Running
                                  </strong>
                                )}
                              ...
                              <td
                                className='cell-actions no-user-select'
                                style={{
                                  padding: '0 10px',
                              ...
                              >
                                ...
                                >
                                  ...
                                  {['TASK_FAILED', 'TASK_COMPLETED'].includes(
                                    pipeline.status
                                  ) && (
                                    <a
                                      href='#'
                                      onClick={() =>
                                        restartPipeline(pipeline.tasks.flat())
                                      }
                                      data-provider={pipeline.id}
                                      className='bp3-button bp3-small bp3-minimal'
                                    >
                          ...
                          <tr>
                            ...
                            >
                              <ContentLoader
                                ...
                                message='Please wait while the data records are loaded.'
                              />
                            </td>
                          </tr>
                        )}
                        ...
                        {!isFetchingAll && filteredPipelines.length === 0 && (
                          <tr>
                            <td
                              className='no-data-message-cell'
                              colSpan='5'
                            ...
                            >
                              ...
                              <p style={{ margin: 0 }}>
                                There are no pipeline logs for the current
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span style={{ color: '#888888' }}>
                        Displaying {filteredPipelines.length === 0 && <>0</>}
                  ...
                  >
                    ...
                    <Button
                      style={{ whiteSpace: 'nowrap' }}
                      disabled={
                        currentPage.current === maxPage || isFetchingAll
                      }
                      onClick={nextPage}
      ...
      {!isFetchingAll && activePipeline && (
        <PipelineIndicator
          ...
          onView={() =>
            history.push(`/pipelines/activity/${activePipeline.ID}`)
          }
          onRetry={() => restartPipeline(activePipeline.tasks)}
        />
      )}
      {!isFetchingAll && inspectPipeline && (
        <CodeInspector
