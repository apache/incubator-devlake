...
import { Providers, ProviderTypes, ProviderLabels, ProviderFormPlaceholders, ProviderIcons } from '@/data/Providers'
...
const ConnectionDialog = (props) => {
  const {
    ...
    dataSourcesList = [
      ...
      {id: 3, name: Providers.GITLAB, title: ProviderLabels[Providers.GITLAB.toUpperCase()], value: Providers.GITLAB},
      {id: 4, name: Providers.JENKINS, title: ProviderLabels[Providers.JENKINS.toUpperCase()], value: Providers.JENKINS},
    ],
      ...
      >
              ...
              <div className='manage-connection'>
                <div className='formContainer'>
                  <FormGroup
                    ...
                    readOnly={[Providers.GITHUB, Providers.GITLAB, Providers.JENKINS].includes(connection.id)}
                    label=''
                    inline={true}
                  ...
                  >
                    ...
                    <InputGroup
                      ...
                      disabled={isTesting || isSaving || isLocked}
                      readOnly={[Providers.GITHUB, Providers.GITLAB, Providers.JENKINS].includes(connection.provider)}
                      placeholder={placeholders ? placeholders.name : 'Enter Instance Name'}
                      value={connection.title}
                      onChange={(e) => onNameChange(e.target.value)}
                      className={`connection-name-input ${stateErrored === 'connection-name' ? 'invalid-field' : ''}`}
                      leftIcon={[Providers.GITHUB, Providers.GITLAB, Providers.JENKINS].includes(connection.provider) ? 'lock' : null}
                      inline={true}
                      rightElement={(
                ...
                <Select
                  ...
                  noResults={<MenuItem disabled={true} text='No data sources.' />}
                  onItemSelect={(item) => {
                    setDatasource(item)
                  }}
        ...
        <div className={Classes.DIALOG_FOOTER}>
          <div className={Classes.DIALOG_FOOTER_ACTIONS}>
            <Button disabled={isSaving || isTesting} onClick={() => onTest(false)}>
              Test Connection
            </Button>
            <Button
