...
var _ core.SubTaskEntryPoint = CollectTaskIssueCommits

func CollectTaskIssueCommits(taskCtx core.SubTaskContext) error {
	data := taskCtx.GetData().(*TapdTaskData)
	db := taskCtx.GetDb()
	logger := taskCtx.GetLogger()
	logger.Info("collect issueCommits")
	cursor, err := db.Model(&models.TapdTask{}).Select("id as issue_id, 'task' as type").
...
var CollectTaskIssueCommitMeta = core.SubTaskMeta{
	Name:        "collectTaskIssueCommits",
	EntryPoint:  CollectTaskIssueCommits,
	Required:    true,
